...
def draw_bounding_box_on_image(current_frame_number,image,
  ...
  #if (roi_info[1] and bottom > roi_info[0]) or ((not roi_info[1]) and left > roi_info[0]): # if the vehicle get in ROI area, vehicle predicted_speed predicted_color algorithms are called - 200 is an arbitrary value, for my case it looks very well to set position of ROI line at y pixel 200
  #      is_vehicle_detected = True
  if roi_info['direction'] == 'top_down' and bottom > roi_info['position']: # if the vehicle get in ROI area, vehicle predicted_speed predicted_color algorithms are called - 200 is an arbitrary value, for my case it looks very well to set position of ROI line at y pixel 200
        is_vehicle_detected, update_csv = speed_prediction.predict_speed(top, bottom, right, left, current_frame_number, detected_vehicle_image, roi_info)
  elif roi_info['direction'] == 'bottom_up' and top < roi_info['position']: # if the vehicle get in ROI area, vehicle predicted_speed predicted_color algorithms are called - 200 is an arbitrary value, for my case it looks very well to set position of ROI line at y pixel 200
        is_vehicle_detected, update_csv = speed_prediction.predict_speed(top, bottom, right, left, current_frame_number, detected_vehicle_image, roi_info)
  elif roi_info['direction'] == 'left_right' and right > roi_info['position']: # if the vehicle get in ROI area, vehicle predicted_speed predicted_color algorithms are called - 200 is an arbitrary value, for my case it looks very well to set position of ROI line at y pixel 200
        is_vehicle_detected, update_csv = speed_prediction.predict_speed(top, bottom, right, left, current_frame_number, detected_vehicle_image, roi_info)
  elif roi_info['direction'] == 'right_left' and left < roi_info['position']: # if the vehicle get in ROI area, vehicle predicted_speed predicted_color algorithms are called - 200 is an arbitrary value, for my case it looks very well to set position of ROI line at y pixel 200
        is_vehicle_detected, update_csv = speed_prediction.predict_speed(top, bottom, right, left, current_frame_number, detected_vehicle_image, roi_info)



#if center of box is pixel____ increase counter
#increase counter+=1
  ...
  csv_line = predicted_color  # csv line created
